...
compSurvs <- function(moic.res         = NULL,
                     surv.info        = NULL,
                     convt.time       = "d",
                     surv.cut         = NULL,
                     xyrs.est         = NULL,
  ...
  if(n.moic > 2) {
    return(list(fitd = fitd, fit = fit, xyrs.est = xyrs, overall.p = p.val, pairwise.p = ps, p))
  } else {
    return(list(fitd = fitd, fit = fit, xyrs.est = xyrs, overall.p = p.val, p))
  }
}
