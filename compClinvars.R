...
compClinvars <- function(moic.res      = NULL,
                        var2comp      = NULL,
                        strata        = NULL,
                        factorVars    = NULL,
                        nonnormalVars = NULL,
  ...
  if(!is.null(warn)) { # if get warning, probably due to the failure of computing exact p value in large data
    set.seed(19991018)
  ...
  if(is.null(tab.name)) {
    if(is.null(strata)){
      outFile <- "Summarization_of_clinical_variables_stratified_by_current_subtype.txt"
    }else{
      outFile <- paste0("Summarization_of_clinical_variables_stratified_by_",strata,".txt")
    }
  } else {
