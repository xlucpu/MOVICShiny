...
runNTPs <- function(expr       = NULL,
  ...
  if(doPlot) {
    outFig <- paste0(file.path(fig.path,fig.name),".pdf")
    pdf(outFig, width = width, height = height)
  ...
  #获取参数列表用于后续绘图
  parameters_list <- list(emat=emat,templates=templates)
  
  return(list(ntp.res = ntp.res, clust.res = ex.moic.res, mo.method = "NTP", parameters_list = parameters_list))
}
