...
compDrugsens <- function(moic.res    = NULL,
                         norm.expr   = NULL,
  ...
  for (drug in drugs) {
    ...
    if(is.null(prefix)) {
      ...
      outFig <- paste0(prefix, "_for_", drug,".pdf")
    }
    ...
    if(n.moic > 2){
      return(list(p=p, predictedBoxdat=predictedBoxdat[[drug]], ic50.test=ic50.test, pairwise.ic50.test=pairwise.ic50.test, test.method = statistic))
    }else{
      return(list(p=p, predictedBoxdat=predictedBoxdat[[drug]], ic50.test=ic50.test, test.method = statistic))
    }
  }
