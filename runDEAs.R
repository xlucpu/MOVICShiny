...
twoclassdeseq2 <- function(moic.res    = NULL,
  ...
  for (k in 1:length(sampleList)) {
    ...
    resData$compname <- rep(compname,nrow(resData))
    
    if(verbose) {
      resData <- resData[,c("id","fc","log2fc","pvalue","padj","compname")]
    } else {
      resData <- resData[,c("id","fc","log2fc","baseMean","lfcSE","stat","pvalue","padj","compname")]
    }
    ...
    if(k==1){
      ...
      dea_results <- rbind(dea_results, resData)
    }
...
twoclassedger <- function(moic.res    = NULL,
  ...
  for (k in 1:length(sampleList)) {
    ...
    if(verbose) {
      ...
      resData <- resData[,c("id","fc","log2fc","logCPM","LR","pvalue","padj","compname")]
    }
    write.table(resData, file = outfile, row.names = FALSE, sep = "\t", quote = FALSE)
    #合并结果
    if(k==1){
...
twoclasslimma <- function(moic.res  = NULL,
  ...
  for (k in 1:length(sampleList)) {
    ...
    if(verbose) {
      ...
      resData <- resData[,c("id","fc","log2fc","t","B","pvalue","padj","compname")]
    }
    write.table(resData, file = outfile, row.names = FALSE, sep = "\t", quote = FALSE)
    #合并结果
    if(k==1){
...
#'
#' @examples # There is no example and please refer to vignette.
runDEAs <- function(dea.method = c("deseq2", "edger", "limma"),
                    expr       = NULL,
                    moic.res   = NULL,
                    prefix     = NULL,
                    overwt     = FALSE,
  ...
  if(method %in% c("deseq2", "edger")) {
    ...
    dea_result <- rundea(moic.res    = moic.res,
                         countsTable = expr,
                         prefix      = prefix,
                         overwt      = overwt,
                         sort.p      = sort.p,
  ...
  return(list(dea_result=dea_result))
}
