...
compFGAs <- function(moic.res     = NULL,
                     segment      = NULL,
                     iscopynumber = FALSE,
                     cnathreshold = 0.2,
  ...
  for (i in 1:length(unique(segment$sample))) {
    ...
    if (length(tmp[abs(tmp$value) > cnathreshold,"bases"]) == 0) {
      FGA = 0
    } else {
      FGA = sum(tmp[abs(tmp$value) > cnathreshold,"bases"]) / sum(tmp[,"bases"])
    }
    
    if (length(tmp[tmp$value > cnathreshold,"bases"]) == 0) {
      FGG = 0
    } else {
  ...
  if(n.moic > 2) {
    return(list(summary = outTab,
                pal = pal, #导入查看该绘图结果前需要先library(patchwork)
